...
vec4 round(float dist) {
    return vec4(1.0, 1.0, 1.0, 1.0 - smoothstep(0.0, smoothFactor, -dist));
}

vec4 gradient(vec4 c1, vec4 c2, vec4 c3, vec4 c4, vec2 pos) {
...
void main() {
    ...
    vec4 color = gradient(colorIn1, colorIn2, colorIn3, colorIn4, pos);
    vec4 outlineColor = gradient(colorOut1, colorOut2, colorOut3, colorOut4, pos);

    float dist = dist(pos);
    float t = clamp(smoothstep(0.0, smoothFactor, -abs(dist) + outlineWidth), 0.0, 1.0);

    gl_FragColor = mix(color, outlineColor, t) * round(-dist);
}
