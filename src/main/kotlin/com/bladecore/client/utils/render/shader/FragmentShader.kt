...
import com.bladecore.client.utils.render.ColorUtils.a
...
class FragmentShader(fragment: String, vertex: String) {
    ...
    private fun createShader(path: String, type: Int): Int {
        ...
        BladeCore.LOG.error("Failed to compile shader!")
        BladeCore.LOG.error("Shader: $path")
        BladeCore.LOG.error("GL Output:")
        BladeCore.LOG.error(GL20.glGetShaderInfoLog(id, 1024))
    ...
    init {
        ...
        if (linked == 0) {
            BladeCore.LOG.error(GL20.glGetProgramInfoLog(id, 1024))
